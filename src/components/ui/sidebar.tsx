...
function SidebarResizeHandle({
  ...
  const { setWidth, state, isMobile, isResizing, setIsResizing } = useSidebar();
  const resizeRef = React.useRef<HTMLDivElement>(null);

  React.useEffect(() => {
    if (!isResizing || isMobile || state === "collapsed") return;
  ...
  return (
    ...
    <div
      ...
      className={cn(
        "absolute inset-y-0 right-0 z-30 w-1 cursor-col-resize bg-transparent transition-colors hover:bg-sidebar-border/80 group-hover:bg-sidebar-border/50",
