...
async fn extract_cover_art(file_path: String) -> Result<Option<String>, String> {
  ...
  // Read the audio file
  let tagged_file =
    read_from_path(&path).map_err(|e| format!("Failed to read audio file: {}", e))?;
...
#[tauri::command]
async fn extract_cover_art_from_bytes(
  file_bytes: Vec<u8>,
  file_name: String,
  ...
  fs::write(&temp_file_path, file_bytes).map_err(|e| {
    format!(
      "extract_cover_art_from_bytes: Failed to write temp file: {}",
      e
    )
  })?;
